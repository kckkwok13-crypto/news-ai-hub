...
export default function HealthArticleContent({ slug }: HealthArticleContentProps) {
  ...
  const renderContent = (content: string) => {
    ...
    let processed = contentWithoutCharts
      ...
      .map((block, i) => {
        ...
        if (block.startsWith('> ')) {
          return <blockquote key={i} className="border-l-4 border-emerald-500 pl-4 py-2 my-4 text-slate-300 italic bg-slate-800/30 rounded-r-lg" dangerouslySetInnerHTML={{ __html: block.replace('> ', '') }} />;
        }
        if (block.startsWith('- ')) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <header className="relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 py-16">
          <Link href="/health" className="inline-flex items-center gap-2 text-emerald-400 hover:text-emerald-300 transition-colors mb-6">
            <ArrowLeft size={20} />
            <span>返回健康養生</span>
          </Link>
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            {post.tags.slice(0, 4).map((tag) => (
              <span key={tag} className="text-xs px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                {tag}
              </span>
            ))}
          </div>
      ...
      <article className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="flex items-center justify-between mt-12 pt-8 border-t border-slate-800">
          <button
            onClick={toggleSaved}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-colors ${isSaved ? 'bg-emerald-500 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
          >
