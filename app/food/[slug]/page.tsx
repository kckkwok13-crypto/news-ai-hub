...
export default function FoodArticlePage() {
  ...
  const renderContent = (content: string) => {
    let processed = content
      ...
      // 🎯 目標框
      .replace(/!\[GOAL\]\((.+?)\|(.+?)\)/g, '<div class="bg-pink-500/10 border border-pink-500/30 rounded-xl p-5 my-6 flex items-center gap-4"><div class="text-3xl">🎯</div><div><h4 class="text-pink-400 font-bold mb-1">$1</h4><p class="text-slate-300 text-sm">$2</p></div></div>')
      // 📷 圖片
      ...
      // 📈 進度條
      .replace(/!\[PROGRESS\|(.+?)\|(.+?)\|(.+?)\]/g, '<div class="my-6"><div class="flex justify-between mb-2"><span class="text-slate-300 text-sm">$1</span><span class="text-rose-400 text-sm font-bold">$2%</span></div><div class="h-3 bg-slate-800 rounded-full overflow-hidden"><div class="h-full bg-gradient-to-r from-rose-500 to-pink-400 rounded-full transition-all duration-500" style="width: $2%"></div></div><p class="text-slate-400 text-xs mt-1">$3</p></div>')
      // 📋 步驟指示器
      ...
      .map(block => {
        ...
        // 引用
        if (block.startsWith('> ')) {
          return `<blockquote class="border-l-4 border-rose-500 pl-5 py-4 my-6 bg-rose-500/10 rounded-r-xl"><p class="text-rose-300 italic leading-relaxed">${block.replace('> ', '')}</p></blockquote>`;
        }
        // 無序列表
        if (block.match(/^- /m)) {
          ...
          const listItems = items.map(line => {
            ...
            return `<li class="flex items-start gap-3 mb-3 text-slate-300"><span class="text-rose-400 mt-1 flex-shrink-0">•</span><span>${text}</span></li>`;
          }).join('');
        ...
        if (block.match(/^\d+\. /m)) {
          ...
          const listItems = items.map(line => {
            ...
            if (match) {
              return `<li class="flex items-start gap-3 mb-3 text-slate-300"><span class="text-rose-400 font-bold min-w-[28px]">${match[1]}.</span><span>${match[2]}</span></li>`;
            }
            return '';
          }).join('');
    ...
    // 處理粗體
    processed = processed.replace(/\*\*(.+?)\*\*/g, '<strong class="text-white font-semibold bg-rose-500/20 px-1.5 py-0.5 rounded">$1</strong>');
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-slate-900/90 border-b border-slate-800">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          >
            <ArrowLeft size={20} />
            <span className="hidden sm:inline">返回美食天地</span>
          </Link>
      ...
      <div className="relative">
        ...
        <div className="relative max-w-4xl mx-auto px-4 pt-16 pb-8">
          <div className="flex items-center gap-2 mb-4">
            <span className="bg-gradient-to-r from-rose-500 to-pink-600 text-white text-xs font-bold px-3 py-1 rounded-full">
              {post.icon} 美食天地
            </span>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mt-12 pt-8 border-t border-slate-800">
          ...
          <div className="flex flex-wrap gap-2">
            {post.tags.map((tag) => (
              <span
                key={tag}
                className="px-3 py-1.5 rounded-full bg-slate-800 text-slate-300 text-sm border border-slate-700 hover:border-rose-500/50 hover:text-rose-400 transition-colors cursor-pointer"
              >
                #{tag}
              </span>
            ))}
        ...
        {/* Action Buttons */}
        <div className="mt-12 p-6 rounded-2xl bg-gradient-to-r from-rose-900/30 to-pink-900/30 border border-rose-500/20">
          <h3 className="text-white text-xl font-bold mb-3">覺得有幫助？</h3>
          <p className="text-slate-400 mb-4">分享俾鐘意美食嘅朋友，或者收藏慢慢睇！</p>
          <div className="flex flex-wrap gap-3">
            <button
              onClick={handleShare}
              className="flex items-center gap-2 px-5 py-2.5 bg-rose-500 text-white rounded-xl font-medium hover:bg-rose-600 transition-colors"
            >
              <Share2 size={18} />
              分享文章
            </button>
            ...
            {foodPosts
              ...
              .map((relatedPost) => (
                <Link
                  ...
                  href={`/food/${relatedPost.slug}`}
                  className="group block bg-slate-800/50 rounded-2xl overflow-hidden border border-slate-700/50 hover:border-rose-500/40 transition-all"
                >
                  <div className="relative h-32 overflow-hidden">
                    <img
                      ...
                      onError={(e) => {
                        e.currentTarget.src = "https://images.unsplash.com/photo-1563245372-f21724e3856d?w=800&q=80";
                      }}
                    />
                  ...
                  <div className="p-4">
                    <h4 className="text-white font-semibold text-sm group-hover:text-rose-400 transition-colors line-clamp-2 mb-2">
                      {relatedPost.title}
                    </h4>
      ...
      <footer className="mt-16 py-10 border-t border-slate-800 text-center">
        <Link href="/food" className="inline-flex items-center gap-2 text-rose-400 hover:text-rose-300 transition-colors">
          <ArrowLeft size={18} />
          <span>返回美食天地</span>
        </Link>
        <p className="text-slate-500 text-sm mt-4">NewsFlow · 美食天地 © 2026</p>
      </footer>

      {/* Share Toast */}
      {showShareToast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-rose-500 text-white px-6 py-3 rounded-full shadow-lg animate-bounce">
          已複製連結 ✓
        </div>
      )}
    </div>
