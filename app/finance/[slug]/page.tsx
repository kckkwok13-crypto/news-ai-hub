...
export default function FinanceArticlePage() {
  ...
  const renderContent = (content: string) => {
    let html = content
      ...
      // 代碼塊
      .replace(/```([\s\S]*?)```/g, '<pre class="bg-slate-900 rounded-xl p-4 my-4 overflow-x-auto"><code class="text-emerald-400 text-sm">$1</code></pre>')
      // 段落
      .replace(/\n\n/g, '</p><p class="text-slate-300 leading-relaxed mb-4">')
      .replace(/^(?!<[h|b|p|l|t|c|>|"])(.+)$/gm, '<p class="text-slate-300 leading-relaxed mb-4">$1</p>');
