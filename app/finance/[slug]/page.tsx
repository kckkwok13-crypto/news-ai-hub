...
export default function FinanceArticlePage() {
  ...
  const renderContent = (content: string) => {
    ...
    let processed = contentWithoutCharts
      ...
      // 🎯 目標框
      .replace(/!\[GOAL\]\((.+?)\|(.+?)\)/g, '<div class="bg-teal-500/10 border border-teal-500/30 rounded-xl p-5 my-6 flex items-center gap-4"><div class="text-3xl">🎯</div><div><h4 class="text-teal-400 font-bold mb-1">$1</h4><p class="text-slate-300 text-sm">$2</p></div></div>')
      // 📷 圖片
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-slate-900/90 border-b border-slate-800">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          >
            <ArrowLeft size={20} />
            <span className="hidden sm:inline">返回理財攻略</span>
          </Link>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mt-12 p-6 rounded-2xl bg-gradient-to-r from-emerald-900/30 to-teal-900/30 border border-emerald-500/20">
          ...
          <p className="text-slate-400 mb-4">分享俾想做投資嘅朋友，或者收藏慢慢睇！</p>
      ...
      <footer className="mt-16 py-10 border-t border-slate-800 text-center">
        <Link href="/finance" className="inline-flex items-center gap-2 text-emerald-400 hover:text-emerald-300 transition-colors">
          <ArrowLeft size={18} />
          <span>返回理財攻略</span>
        </Link>
        <p className="text-slate-500 text-sm mt-4">NewsFlow · 理財攻略 © 2026</p>
      </footer>
