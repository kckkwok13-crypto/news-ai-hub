...
export default function FinanceArticleContent({ slug }: FinanceArticleContentProps) {
  const post = financePosts.find((p) => p.slug === slug);
  ...
  const renderContent = (content: string) => {
    ...
    let processed = contentWithoutCharts
      ...
      .replace(/!\[KEYPOINT\]\((.+?)\)/g, '<div class="bg-teal-500/10 border border-teal-500/30 rounded-xl p-5 my-6 flex items-start gap-4"><div class="text-3xl">✅</div><div><h4 class="text-teal-400 font-bold mb-2">重點</h4><p class="text-teal-200/80">$1</p></div></div>')
      ...
      .replace(/!\[STEP\|(.+?)\|(.+?)\]/g, '<div class="flex items-center gap-4 my-4 p-4 bg-slate-800/50 rounded-xl border border-slate-700/50"><div class="w-10 h-10 rounded-full bg-teal-500 flex items-center justify-center text-white font-bold text-lg flex-shrink-0">$1</div><div class="text-slate-300">$2</div></div>')
      .split('\n\n')
      .map((block, i) => {
        if (block.startsWith('## ')) {
        ...
        if (block.startsWith('> ')) {
          return <blockquote key={i} className="border-l-4 border-teal-500 pl-4 py-2 my-4 text-slate-300 italic bg-slate-800/30 rounded-r-lg" dangerouslySetInnerHTML={{ __html: block.replace('> ', '') }} />;
        }
        if (block.startsWith('- ')) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <header className="relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 py-16">
          <Link href="/finance" className="inline-flex items-center gap-2 text-teal-400 hover:text-teal-300 transition-colors mb-6">
            <ArrowLeft size={20} />
            <span>返回理財攻略</span>
          </Link>
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            {post.tags.slice(0, 4).map((tag) => (
              <span key={tag} className="text-xs px-3 py-1 rounded-full bg-teal-500/20 text-teal-300 border border-teal-500/30">
                {tag}
              </span>
            ))}
          </div>
      ...
      <article className="max-w-4xl mx-auto px-6 py-12">
        ...
        {/* Charts */}
        <ArticleChart postSlug={post.slug} category="finance" />

        {/* Actions */}
        <div className="flex items-center justify-between mt-12 pt-8 border-t border-slate-800">
          <button
            onClick={toggleSaved}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-colors ${isSaved ? 'bg-teal-500 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
          >
