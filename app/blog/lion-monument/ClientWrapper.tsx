...
const tContent = {
  yue: {
    ...
    duration: "⏱️ 建議遊覽",
    free: "完全免費<br/>無須門票",
    hours24: "24小時全天候開放<br/>免費參觀",
    busInfo: "巴士至 Löwenplatz 站",
    visitTime: "1-2小時",
    ...
    heroImg: "/images/travel/lion-monument-detail.jpg",
...
export default function LionMonumentPage() {
  ...
  const tc = isYue ? tContent.yue : null;
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <header className="text-center py-12">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-white via-gray-200 to-slate-300 bg-clip-text text-transparent">
            {tc ? tc.title : "世界上最悲傷的石頭"}
          </h1>
          <h2 className="text-xl text-gray-400 font-semibold mb-4">{tc ? tc.subtitle : t.meta.subtitle || "瑞士琉森獅子紀念碑（Löwendenkmal）深度遊覽攻略"}</h2>
          <p className="text-gray-600">June 2026 · {tc ? tc.author : t.author}</p>
        </header>
        ...
        <p className="text-center text-gray-600 text-sm mb-12">
          {tc ? tc.heroCaption : "▲ 隱藏在琉森舊城森林幽谷中的垂死獅子紀念碑，馬克·吐溫稱其為「世界上最讓人心碎的雕塑」"}
        </p>
        ...
        <article className="prose prose-invert prose-lg max-w-none">
          <div className="bg-gradient-to-br from-gray-900/50 to-slate-900/40 border border-gray-600/30 rounded-2xl p-6 my-8">
            <p className="text-gray-300 text-lg italic leading-relaxed border-l-4 border-yellow-500/50 pl-6" dangerouslySetInnerHTML={{ __html: tc ? tc.introQuote : "「馬克·吐溫說，這是世界上最讓人心碎、最感人的一塊石頭；而當你站在這面被幽靜森林環繞的巨型花崗岩崖壁前，看著那隻利箭穿心、神態痛苦卻死死用身軀護著波旁王朝盾牌的巨獅，你才會明白：這不是一尊冰冷的雕塑，而是一曲用鮮血與絕對忠誠譜寫的長眠哀歌。」" }} />
          </div>

          <p id="intro" dangerouslySetInnerHTML={{ __html: tc ? tc.introP1 : "如果說卡貝爾橋展現了琉森（Lucerne）溫柔繾綣的湖畔風情，那麼隱藏在老街背後、掩映在蔥鬱樹林之中的<strong>垂死獅子紀念碑（Lion Monument / Löwendenkmal）</strong>，無疑就是這座瑞士山城最沉重、最直擊靈魂的歷史圖騰。這座完全在一面巨型砂岩採石場崖壁上<strong>純手工鑿刻而成</strong>的巨幅雕塑，長 10 米，高 6 米，落成於 1821 年。它不僅是一件卓越的藝術品，更是一座記錄了 1792 年法國大革命期間，一場震驚歐洲的血腥大屠殺的紀念碑。" }} />
          ...
          <h2 id="experience">{tc ? tc.experienceTitle : "🌲 第一身沉浸實感：在萬樹沙沙與綠池如淚間凝望不朽"}</h2>
          ...
          <div className="my-8">
            ...
            <p className="text-center text-gray-600 text-sm mt-4 mb-8">
              {tc ? tc.imgCaption : "▲ 近距離觀看垂死獅子雕像，可見其痛苦神情與盾牌上的百合花紋章"}
            </p>
          </div>
          ...
          <h2>{tc ? tc.infoTitle : t.infoTitle}</h2>
          <div className="grid grid-cols-2 gap-4 my-6">
            <div className="bg-gradient-to-br from-gray-900/50 to-zinc-900/60 rounded-xl p-4 border border-gray-700/30">
              <span className="text-gray-400 font-bold">{tc ? tc.address : t.address}</span>
              <p className="text-gray-100/80 text-sm mt-1" dangerouslySetInnerHTML={{ __html: tc ? tc.addressValue : "Denkmalstrasse 4, 6006 Luzern, Switzerland" }} />
            </div>
            <div className="bg-gradient-to-br from-gray-900/50 to-zinc-900/60 rounded-xl p-4 border border-gray-700/30">
              <span className="text-gray-400 font-bold">{tc ? tc.hours : t.openingHours}</span>
              <p className="text-gray-100/80 text-sm mt-1" dangerouslySetInnerHTML={{ __html: tc ? tc.hours24 : "24小時全天候開放<br/>免費參觀" }} />
            </div>
            <div className="bg-gradient-to-br from-gray-900/50 to-zinc-900/60 rounded-xl p-4 border border-gray-700/30">
              <span className="text-gray-400 font-bold">{tc ? tc.fee : t.fee}</span>
              <p className="text-gray-100/80 text-sm mt-1" dangerouslySetInnerHTML={{ __html: tc ? tc.free : "完全免費<br/>無須門票" }} />
            </div>
            <div className="bg-gradient-to-br from-gray-900/50 to-zinc-900/60 rounded-xl p-4 border border-gray-700/30">
              <span className="text-gray-400 font-bold">{tc ? tc.rating : t.rating}</span>
              <p className="text-gray-100/80 text-sm mt-1">{tc ? tc.ratingValue : "4.8/5.0（28,432 評論）"}</p>
            </div>
            <div className="bg-gradient-to-br from-gray-900/50 to-zinc-900/60 rounded-xl p-4 border border-gray-700/30">
              <span className="text-gray-400 font-bold">{tc ? tc.transport : t.transport}</span>
              <p className="text-gray-100/80 text-sm mt-1">{tc ? tc.busInfo : "巴士至 Löwenplatz 站"}</p>
            </div>
            <div className="bg-gradient-to-br from-gray-900/50 to-zinc-900/60 rounded-xl p-4 border border-gray-700/30">
              <span className="text-gray-400 font-bold">{tc ? tc.duration : t.visitDuration}</span>
              <p className="text-gray-100/80 text-sm mt-1">{tc ? tc.visitTime : "1-2小時"}</p>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-gray-900/30 to-zinc-900/20 border border-gray-600/30 rounded-2xl p-6 my-10">
            <h3 className="text-gray-400 font-bold mb-4 flex items-center gap-2 text-xl">
              {tc ? tc.ratingTitle : t.ratingTitle}
            </h3>
            <StarRating slug="lion-monument" />
          </div>

          <div className="bg-gray-900/30 rounded-2xl p-6 my-10 border border-gray-700/30">
            <h3 className="text-white font-bold mb-4 text-xl">{tc ? tc.shareTitle : t.shareTitle}</h3>
            <SocialShare
          ...
          <div className="flex justify-center my-8">
            <div className="bg-gray-900/30 rounded-2xl p-6 border border-gray-700/30 flex items-center gap-4">
              <span className="text-gray-100/80">{tc ? tc.favoriteText : t.favoriteText}</span>
          ...
          <div className="border-t border-gray-700/30 pt-8 mt-8">
            <p className="text-gray-400 italic text-center">
              {tc ? tc.bottomQuote : "歷史鐫刻於砂岩，忠誠長留於森林。願每位造訪這片綠色幽谷的旅人，都能在琉森的巨獅眼淚中找到屬於自己的思辨夢。"}
            </p>
          </div>
