    ...
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-purple-950 to-zinc-950 text-white">
      ...
      <div className="fixed right-6 top-1/2 -translate-y-1/2 z-40 hidden lg:block">
        <div className="bg-gradient-to-br from-zinc-900/95 to-zinc-800/95 backdrop-blur-xl border border-purple-500/30 rounded-2xl p-5 w-60 shadow-2xl shadow-purple-500/10">
          <h3 className="text-sm font-bold text-purple-400 mb-4 flex items-center gap-2">📋 目錄導覽</h3>
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        <Link href="/blog" className="inline-flex items-center gap-2 text-purple-400 hover:text-white mb-8 transition-colors bg-zinc-800/50 px-4 py-2 rounded-full hover:bg-zinc-700/50">
          ← 返回 Blog
        </Link>
        ...
        <article className="prose prose-invert prose-lg max-w-none">
          ...
          <p>今天呢篇 Blog，就帶你深度攻略呢個世界聞名嘅十字路口，從最佳拍攝位到周邊商場，全部一次過話你知！</p>

          <h2 id="crossing">🚶 為何這個路口如此有名？</h2>
          <p>澀谷十字路口之所以聞名全世界，係因為佢係<strong>全球最繁忙的行人穿越道</strong>。根據估計，每個綠燈周期大約有 3,000 人同時過馬路，而每日更有多達 50 萬人次经过呢度。當所有行人的步伐在同一瞬間由四面八方匯聚，形成一個好似漩渦咁的奇觀，外國旅客第一次見到都會忍不住「嘩」一聲！</p>

          <div className="my-8">
            <img src="https://images.unsplash.com/photo-1503899036084-c55cdd92da26?w=1200&q=80" alt="澀谷十字路口日間人流" className="w-full rounded-2xl" />
            <p className="text-center text-zinc-500 text-sm mt-4 mb-8">▲ 日間的十字路口，來自世界各地的旅客在此交匯</p>
          </div>

          <h2 id="scramble">🎯 隨機應變的「澀谷式過馬路」</h2>
          <p>如果你第一次行呢個路口，可能會有啲不知所措。放心，等我哋教你！因為人流實在太多，東京人發展出一套獨特嘅<strong>「澀谷式過馬路」</strong>技巧：</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
            {[
              { emoji: "🛍️", name: "SHIBUYA109", desc: "東京辣妹時尚的地標，商場充斥着最新潮流款式與限定商品。" },
              { emoji: "🎮", name: "澀谷Scramble Square", desc: "2019年新開的商業大樓，頂層有景觀台可以俯瞰整個十字路口！" },
              { emoji: "🍜", name: "杵屋麵", desc: "百年歷史的日本橋蕎麥麵老店，經濟實惠又好食。" },
              { emoji: "☕", name: "星巴克澀谷店", desc: "二樓有落地玻璃窗，可以270度俯瞰十字路口，是最佳拍攝位！" },
            ].map((item) => (
              <div key={item.name} className="bg-gradient-to-br from-zinc-800/60 to-zinc-900/60 rounded-xl p-4 border border-zinc-700/50">
                <span className="text-purple-400 font-bold">{item.emoji} {item.name}</span>
          ...
          <div className="bg-gradient-to-br from-purple-900/20 to-pink-900/20 border border-purple-500/30 rounded-2xl p-6 my-10" id="tips">
            <h3 className="text-purple-400 font-bold mb-4 flex items-center gap-2 text-xl">💡 澀谷十字路口 旅遊實用小貼士 (Travel Tips)</h3>
            <ul className="space-y-3">
              <li className="flex gap-3">
                <span className="text-purple-400">📸</span>
                <span className="text-zinc-300"><strong>最佳拍攝位：</strong>推薦去<strong>星巴克澀谷中央街店二樓</strong>，有落地大玻璃窗，可以影到成個十字路口的全景。</span>
              </li>
              <li className="flex gap-3">
                <span className="text-purple-400">🌙</span>
          ...
          <div className="grid grid-cols-2 gap-4 my-6">
            {[
              { label: "📍 地址", value: "Shibuya Station, Shibuya, Tokyo" },
              { label: "🕐 開放時間", value: "24小時開放" },
              { label: "💰 費用", value: "免費參觀" },
              { label: "⭐ 評分", value: "4.5/5.0（89,234 評論）" },
              { label: "🚇 交通", value: "東京Metro 澀谷站 步行1分鐘" },
              { label: "⏱️ 建議遊覽", value: "1-2小時" },
            ].map((item) => (
              <div key={item.label} className="bg-gradient-to-br from-zinc-800/60 to-zinc-900/60 rounded-xl p-4 border border-zinc-700/50">
                <span className="text-purple-400 font-bold">{item.label}</span>
          ...
          <div className="bg-gradient-to-r from-purple-900/30 to-pink-900/20 border border-purple-500/30 rounded-2xl p-6 my-10">
            <h3 className="text-purple-400 font-bold mb-4 flex items-center gap-2 text-xl">💬 留言分享</h3>
            <p className="text-zinc-300 text-lg mb-4">👇 你試過在澀谷十字路口被「人流漩渦」淹沒過嗎？分享一下你的體驗！</p>
            <div className="space-y-3">
              <BlogComments theme="dark_dimmed" lang="zh-TW" />
            </div>
          </div>
        </article>
      </div>
